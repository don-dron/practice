...
Задача описывается в терминах входных наблюдений, искомых преобразований и показателей качества. Формализация охватывает полностраничный архивный случай с возможными отказами структуры макета и деградацией носителя.
...
Объединение трёх источников в единый `torch.utils.data.Dataset` или эквивалент выполняется через адаптеры форматов: единый тип записи изображение или crop строки, эталонная строка или полигон, идентификатор домена (источника набора) и версии предобработки; конкретная реализация приводится в репозитории проекта и журнале конфигураций эксперимента (§5).
...
На этапе S5 сохранённые весовые параметры torchvision ResNet‑18 задают инициализацию фильтробанка нижней части дорожки; снижение потери в первые две замороженные эпохи соответствует фазе без обновления параметров кодировочной части, после чего в эпохи 3–5 наблюдается догон по сходимости относительно T0. Финально потери оказываются близки к T0 без гарантированного преимущества на условном столбце символьной ошибки в рамках пяти эпох полного набора наблюдений; интерпретация на более длинном горизонте обучения выходит за рамки настоящего протокола.
...
Частичная разведка по гиперпараметрам без полной решётки комбинаций; таблица и рисунок задают ориентировочную чувствительность условной символьной ошибки `ŜER*` и часов прогона при восстановлении чисел из экспортов по тому же протоколу, что в п. 5.2–6.3.
...
Структура наблюдений разделена на воспроизводимый таксон ошибок при неизменении жадного декодирования и при зафиксированной геометрической связке строки и ограничивающего прямоугольника теми же разбиениями COCO, что упомянуты в пп. 3–5 методики набора госотчётов.
...
| Соответствие домена данных гл. 3 | 71 | 73 |
...
В работе рассмотрена задача строкового распознавания текста как отображение фрагмента изображения строки в последовательность символов при подготовке данных в форме COCO-аннотаций (`coco_lines`) и обучении на PyTorch связок CRNN+LSTM+CTC, авторегрессии с перекрёстной энтропией, трансформерного кодировщика под CTC и трансферного варианта с предобученным блоком кодирования до BiLSTM+CTC в составе перечня экспериментальных конфигураций главы 5. Условия воспроизводимости: единое слияние YAML-конфигураций, сохранение идентификаторов эксперимента с контрольной точкой, воспроизводимость ресайза и вырезки строки; иллюстративные ряды глав 5–6 строятся из экспортированных таблиц зафиксированных прогонов сценариями построения без произвольной правки узлов вне исходных CSV. Репозиторий связывает обучение с интерфейсом командной строки (`htr-train`), жадную декодировку с конвейером вывода и поштучную проверку поля через `infer_one_coco_line.py`. Табличные файлы и журналы прогонов содержат метрики тех же сеансов обучения и проверки сохранённых моделей и восстанавливают приведённые кривые и столбцы без расхождения с протоколом.

Основным инженерно-исследовательским итогом принимается связка п. 4.1, конфигурируемая по умолчанию как T0 в файле `configs/default.yaml`: она укладывается в ограниченный объём видеопамяти рабочей станции порядка 8 Гбайт при фиксируемом мини-пакете с накоплением шестнадцати шагов, задаёт сходимость CTC-потерь без посимвольной временной разметки учителя и служит опорой при сравнении со связкой S3 по времени обучения на графическом ускорителе и по условной символьной ошибке на согласованной отложенной подвыборке в материале главы 5. Связки S2, S5 и страничный контур S4 задают границы протоколов и разграничение шкал качества между блоком без механизма внимания и постановками с авторегрессией. Типовые паттерны ошибок сопоставлены с ограничениями жадной декодировки и точности ограничивающего прямоугольника строки при выборочной верификации.
...
Воспроизводимый контур экспериментов и выбор опорной строковой связки распознавания текста в конфигурируемом проекте на материале набора Yenisei Gov Reports TD и формата `coco_lines`, с методическим сравнением с более тяжёлыми архитектурами при росте вычислительного бюджета.
