...
Подраздел описывает набор экспериментальных конфигураций для строковых моделей согласно реализации практики. Параметры задаются файлами YAML в каталогах `configs/` и `configs/baselines/` и объединяются в один словарь в порядке передачи аргументов утилиты обучения (п. 5.1.1). В регистре (п. 5.1.2) для каждого варианта заданы символьное имя эксперимента, отсылка к подразделу §4, признак того, выполняется ли в текущем коде цикл оптимизации параметров модели по обучающей выборке, и упорядоченный список путей к YAML.

Исполнимой считается конфигурация, для которой в программе есть собранный вычислительный граф и выполнен полный проход минимизации по обучающей выборке. Связки T0, E1 и эквиваленты описывают §4.1 (CNN, BiLSTM, CTC); профили §4.2 реализуются как строковый энкодер‑декодер с additive attention при `training.objective: attention_ce` и `model.name: attention_line_seq2seq`; §4.3 как трансформерный кодировщик над CNN-признаками строки и головкой CTC (`transformer_encoder_line_ctc`). Для §4.4 доступен воспроизводимый только строковый этап на `coco_lines` (детектор страницы пока без отдельного обучателя); связка задаёт тем же файлом упрощённый вариант того же кодировочного контура §4.3. Для §4.5 граф использует упрощённый stem и начальные блоки предобученного ImageNet **ResNet-18**, поверх них BiLSTM и CTC; политика затирания кодировочной части на первые эпохи задаётся `planned_transfer_policy.freeze_encoder_until_epoch` или `model.freeze_backbone_epochs`.

Отличие профиля E1 от файла `configs/default.yaml` после слияния сведено в п. 5.1.3. Однотипные для всех исполнимых связок параметры оптимизатора и функции потерь указаны в п. 5.1.4.
...
- Если хотя бы один `-c` задан явно, в смешении участвуют только перечисленные файлы; `configs/default.yaml` не добавляется автоматически вторым шагом. Связка «база + профиль» задаётся явно двумя (или более) аргументами `-c`, например сначала `configs/default.yaml`, затем файл из `configs/baselines/`.
- Если ни один аргумент `-c` не указан, обучитель считывает один файл `configs/default.yaml` (соответствие строке регистра T0, п. 5.1.2).

Число эпох, размер минипакета и устройство могут быть заданы параметрами командной строки и в этом случае заменяют соответствующие ключи финального YAML после слияния файлов. Семянное число задаётся в файлах YAML ключом `project.seed`.

В чекпоинте сохраняется конфигурация в виде словаря после слияния YAML и параметров командной строки на момент сохранения.

#### 5.1.2 Регистр экспериментов

В таблице статус «обучение» означает, что выполнен полный проход минимизации: для связок на CTC — `CTCLoss`; для S2 дополнительно используется кросс-энтропия над символами с явным teacher forcing. Статус «частично» означает, что реализована излагаемая в §4 вторичная задача строки без полномасштабного обучения детектора страниц (случай связки §4.4 при текущих данных).

| Обозначение | Связь с §4 | Статус | Цепочка YAML в порядке `-c`, поле имени эксперимента после слияния | Комментарий |
|---------------|--------------|----------|------------------------------------------------------------|---------------|
| T0 | связка по умолчанию в `configs/default.yaml` без альтернативной гипотезы §4 | обучение | один файл `configs/default.yaml` или вызов без `-c`; поле `training.experiment_name`: `run` | сравнение с E1 при прочих равных условиях |
| E1 | §4.1; выборка Digital Peter; инициализация без внешнего чекпоинта | обучение | `configs/default.yaml`, `configs/baselines/4_01_line_crnn_ctc_digital_peter.yaml`; поле `training.experiment_name`: `dpeter_crnn_ctc`; допускаются ключи из командной строки | основной исполнимый вариант §4.1 в составе репозитория |
| E1\* | §4.1; то же, что E1 | обучение | `configs/default.yaml`, `configs/digital_peter_line_ctc.yaml`; `training.experiment_name`: `dpeter_crnn_ctc` | копия профиля E1 в корне `configs/` |
| E2 | §4.1; три файла слияния | обучение | `configs/default.yaml`, `configs/baselines/overlay_comparable_line_budget.yaml`, `configs/baselines/4_01_line_crnn_ctc_digital_peter.yaml` | единый формат цепочки для согласования бюджета с будущими профилями; при неизменном содержании YAML совпадает с E1 численно |
| S2 | §4.2, энкодер–декодер с вниманием | обучение | `configs/default.yaml` затем `configs/baselines/4_02_line_stub_encoder_decoder_attention.yaml` | `attention_line_seq2seq`, `training.objective: attention_ce` |
| S3 | §4.3, трансформерный кодировщик строки + CTC | обучение | `configs/default.yaml` затем `configs/baselines/4_03_line_stub_transformer_encoder_attention.yaml` | `transformer_encoder_line_ctc` |
| S4 | §4.4, план полностраничной связки при текущих данных только строка | частично | `configs/default.yaml` затем `configs/baselines/4_04_page_stub_detector_then_line.yaml` | тот же тип графа, что в S3 с уменьшенным размерностным бюджетом; ключи под детектор остаются плейсхолдером |
| S5 | §4.5, предобученный визуальный кодировочный контур до BiLSTM+CTC | обучение | `configs/default.yaml`, `configs/baselines/4_05_line_stub_transfer_pretrained_encoder.yaml` | `pretrained_resnet_line_ctc`, веса ImageNet через torchvision и политика заморозки в YAML (`planned_transfer_policy` / `model.freeze_backbone_epochs`) |

Команды для исполнимых связок T0, E1, E1\*, E2 и S2–S5 (при необходимости дополнить параметрами командной строки):

```
htr-train --config configs/default.yaml

htr-train --config configs/default.yaml --config configs/baselines/4_01_line_crnn_ctc_digital_peter.yaml

htr-train --config configs/default.yaml --config configs/digital_peter_line_ctc.yaml

htr-train --config configs/default.yaml --config configs/baselines/overlay_comparable_line_budget.yaml --config configs/baselines/4_01_line_crnn_ctc_digital_peter.yaml

htr-train --config configs/default.yaml --config configs/baselines/4_02_line_stub_encoder_decoder_attention.yaml

htr-train --config configs/default.yaml --config configs/baselines/4_03_line_stub_transformer_encoder_attention.yaml

htr-train --config configs/default.yaml --config configs/baselines/4_04_page_stub_detector_then_line.yaml

htr-train --config configs/default.yaml --config configs/baselines/4_05_line_stub_transfer_pretrained_encoder.yaml
```

#### 5.1.3 Различия ключей финального YAML: T0 против E1

Рассматриваются два слияния: только `configs/default.yaml` (T0) и `configs/default.yaml` с файлом профиля E1 из `configs/baselines/4_01_line_crnn_ctc_digital_peter.yaml`; содержание последнего совпадает с файлом `configs/digital_peter_line_ctc.yaml`. В таблице перечислены ключи, значения которых различаются. Остальные параметры (список каналов CNN, число слоёв рекуррентного блока, нулевое значение `dropout` из базового файла, `img_height`, отключённая аугментация загрузки, сбор алфавита по обучающей выборке и т. д.) сохраняют значения после слияния с первым файлом, если второй YAML этих ключей не содержит.

| Ключ конфигурации | значение при T0 | значение при E1 с профилем `configs/baselines/4_01_line_crnn_ctc_digital_peter.yaml` |
|-------------------|-------------------|------------------------------------------------------------------------------------------------|
| `data.max_width` | 1200 | 900 |
...
#### 5.1.4 Общие параметры процедуры оптимизации по связкам со статусом «обучение»

Для указанных обозначений в коде задаётся оптимизатор класса AdamW (PyTorch), темп из `training.lr`, весовое затухание из `training.weight_decay`, отсечение градиента по порогу `training.clip_grad_norm`, при необходимости смешанная точность `training.amp`. Для режимов CTC целевая функция задаётся как `CTCLoss(zero_infinity=True)` при выходном распределении `log_softmax` по временным шагам. Для S2 выполняется кросс-энтропия по авторегрессионным шагам декодера с teacher forcing поверх упорядоченных индексов символов; на валидации используется жадное авторегрессионное продолжение до EOS или ограничения `training.decoder_max_steps`. Экспериментальное разбиение наблюдений: случайная доля `data.val_fraction`.

При инференсе высота строки и ограничение ширины определяются полями `data` упакованного чекпоинта или — при передаче цепочки `-c` — словарём после слияния YAML утилиты вызова.

### 5.2 Обучение и сравнение моделей

Подраздел задаёт выполняемые при практической реализации процедуры обучения и ограничения на сравнение режимов из п. 5.1.2 при различном составе модели потерь.

На момент написания текста полный проход минимизации реализован для связок регистра T0, E1, E1*, E2 и S2–S5 (со статусом «обучение» или частичным для связки страницы, п. 5.1.2). Справедливость прямых количественных сравнений между семействами постановок требует согласовать бюджет эпох, размер минипакета, ширину кропов, алфавит и правила символьной метрики: для режимов CTC сопоставимо жадное выравнивание по времени, для режима §4.2 допускаются иные ошибки из-за авторегрессионного построения гипотезы.

Сравнение исполнимых конфигураций опирается на следующие ограничения неизменности. Обучение и случайная валидационная доля формируются по одному COCO-датасету; при сравнении T0 и E1 в связке для Digital Peter (п. 5.1) ключи `coco_json` и `image_root` не меняются. Различия между T0 и E1 задаются гиперпараметрами обучения, ограничением ширины кропов и долей объектов под валидацию, но не файлом наблюдений. Изменение числа эпох, размера минипакета или устройства через командную строку фиксируется в сохранённой копии конфигурации в чекпоинте наравне со слиянием YAML; при необходимости воспроизводимости сравнений эти параметры задаются одинаковыми для сопоставляемых прогонов.

Отдельную роль выполняют контролирующие наблюдения внутри каждого прогона. В реализации обучителя после каждой эпохи вычисляется среднее значение функции потерь на минипакетах обучающего множества; на валидации для конфигураций на CTC строится грубое жадное выравнивание по времени без лучевого поиска, для режима §4.2 выполняется жадное авторегрессионное продолжение до EOS или лимита `training.decoder_max_steps`. Выбор контрольной точки сохранения параметров может опираться на минимизацию указанной валидационной величины по эпохам либо, при равномерном сохранении по расписанию, на последнюю сохранённую точку эксперимента при фиксации её в тексте результата. Дискриминацию между постановками §4.1–§4.5 допускается проводить на одном наборе символьных и словных метрик из §2 при согласованных правилах нормализации текста и, по возможности, на одной тестовой выборке вне разбиения этого датасета; конкретизация таблицы и графиков отнесена к разделу 6.

При появлении двухступенчатого контура §4.4 сравнение разделяется на качество локализации зон текста по доступным данным и на качество строкового восстановления на вырезках по истинной разметке и по выходу детектора, чтобы разделить вклад локализации и вклад строкового распознавателя. Оценка инициализации кодировочной части весами внешней задачи OCR в режиме §4.5 задаётся как изменение только политики инициализации и режима обучения при прочих равных элементах протокола; источник внешних весов фиксируется в журнале эксперимента и в конфигурации, сохранённой в чекпоинте.

Итого в настоящем подразделе сравнение понимается как воспроизводимое обучение по цепочкам из п. 5.1.2 для обозначений со статусом обучение и как явно зафиксированный перечень факторов, которые между сопоставляемыми прогонами не меняются либо меняются намеренно. Численные показатели качества после обучения и инференса приводятся в разделе 6; доработка гиперпараметров лучшей конфигурации — в п. 5.3.
