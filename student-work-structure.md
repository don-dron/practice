...
Исполнимой считается конфигурация, для которой в программе реализован полный конвейер обучения архитектуры §4.1 (свёрточное извлечение признаков, двунаправленная одномерная рекуррентная дорожка, потеря CTC). Конфигурации, отнесённые к §4.2–§4.5, в текущей версии кода сохранены в YAML как текстовые спецификации ключей без соответствующей сборки графа; при их вызове обучатель завершает работу до шага оптимизации и выводит диагностическое сообщение.
...
В таблице статус «обучение» соответствует конфигурациям, для которых выполняется цикл минимизации потерь на обучающих данных модели §4.1 (CNN, двунаправленная рекуррентная дорожка, потеря CTC). Статус «документ» назначён конфигурациям постановок §4.2–§4.5 без реализованного в коде графа; при попытке обучения утилита завершается до оптимизации и выводит сообщение об отсутствии реализации.
...
| S2 | §4.2, энкодер–декодер с вниманием | документ | `configs/default.yaml` затем `configs/baselines/4_02_line_stub_encoder_decoder_attention.yaml` | поля описания намеченной топологии вынесены в `planned_model` |
| S3 | §4.3, кодировочный контур через механизм многоточечного самосопоставления | документ | `configs/default.yaml` затем `configs/baselines/4_03_line_stub_transformer_encoder_attention.yaml` | аналогично предыдущей строке |
| S4 | §4.4, двухступенчатая связка локализации и строки | документ | `configs/default.yaml` затем `configs/baselines/4_04_page_stub_detector_then_line.yaml` | в этом YAML выделены заделочные ключи под отдельный полностраничный обучатель |
| S5 | §4.5, дообучение с внешней инициализацией кодировочной части | документ | `configs/default.yaml`, `configs/baselines/4_05_line_stub_transfer_pretrained_encoder.yaml` | в YAML указан только каркас; путь к внешним весам пустой; статус может быть изменён на «обучение» после добавления загрузки в код |

Команды для исполнимых связок T0, E1, E1\* и E2 (при необходимости дополнить параметрами `--epochs`, `--device`, `--batch-size`):
...
#### 5.1.4 Общие настройки обучения для связок со статусом «обучение» (T0, E1, E1*, E2)

Для указанных обозначений в текущей реализации задаются следующие параметры процедуры оптимизации. Оптимизатор класса AdamW (библиотека PyTorch), темп из `training.lr`, весовое затухание из `training.weight_decay`. Целевая функция: `CTCLoss(zero_infinity=True)`. Отсечение градиента по глобальной норме: порог `training.clip_grad_norm`. Вычисления в смешанной точности: по флагу `training.amp`, по умолчанию выключено. Экспериментальное разбиение наблюдений: случайная доля `data.val_fraction` при отсутствии выделенного JSON валидации.
...
Подраздел задаёт выполняемые при практической реализации процедуры обучения и правила того, какие режимы из перечисленных в п. 5.1.2 допускают сравнение между собой в текущем состоянии кода, а какие остаются вне этого сравнения до подключения соответствующих графов вычислений.

На момент написания текста полный цикл оптимизации параметров строковой модели реализован только для связок со статусом «обучение» (обозначения T0, E1 и эквивалент E1*, а также связка E2, численно совпадающая с E1 при неизменном содержании YAML после слияния, п. 5.1.2). Связки S2–S5 не участвуют в экспериментальном протоколе сравнения качества моделей до появления реализации архитектур §4.2–§4.5 в обучителе; они учитываются только как зафиксированные параметрические шаблоны для последующего продолжения работ.

Сравнение исполнимых конфигураций опирается на следующие ограничения неизменности. Обучение и случайная валидационная доля формируются по одному COCO-датасету; при сравнении T0 и E1 в связке для Digital Peter (п. 5.1) ключи `coco_json` и `image_root` не меняются. Различия между T0 и E1 задаются гиперпараметрами обучения, ограничением ширины кропов и долей объектов под валидацию, но не файлом наблюдений. Процедура оптимизатора, функция потерь CTC и механизм отсечения градиента совпадают между исполнимыми связками (п. 5.1.4). Изменение числа эпох, размера минипакета или устройства через командную строку фиксируется в сохранённой копии конфигурации в чекпоинте наравне со слиянием YAML; при необходимости воспроизводимости сравнений эти параметры задаются одинаковыми для сопоставляемых прогонов.

Отдельную роль выполняют контролирующие наблюдения внутри каждого прогона. В реализации обучителя после каждой эпохи вычисляется среднее значение функции потерь на минипакетах обучающего множества; на случайной валидационной доле, если она не пуста после разбиения, дополнительно оценивается грубая символьная метрика согласованности распознанного текста относительно эталона (жадное декодирование по временной оси CTC без лучевого поиска). Выбор контрольной точки сохранения параметров может опираться на минимизацию указанной валидационной величины по эпохам либо, при равномерном сохранении по расписанию, на последнюю сохранённую точку эксперимента при фиксации её в тексте результата. Дискриминацию моделей вида §4.1 и постановок §4.2–§4.5 после их реализации предполагается проводить на одном наборе символьных и словных метрик из §2 при единых правилах нормализации текста и по возможности на одной тестовой выборке, не пересекающейся с обучением и с внутри-датасетной валидацией; конкретизация отчёта по метрикам отнесена к разделу 6.
