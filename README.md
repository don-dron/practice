...
htr-train --config configs/default.yaml --epochs 5 --device cpu
...
```bash
htr-train --config configs/default.yaml --config configs/baselines/4_01_line_crnn_ctc_digital_peter.yaml --epochs 5 --device cpu
```

Без установки editable (пример только Yenisei):

```bash
PYTHONPATH=src python -c "import sys; from htr.cli import train_main; sys.argv=['htr-train']+sys.argv[1:]; train_main()" --config configs/default.yaml --epochs 5 --device cpu
