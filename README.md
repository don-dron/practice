...
```bash
htr-train --config configs/default.yaml --config configs/digital_peter_line_ctc.yaml --epochs 60 --device cpu
```

Без установки editable:

```bash
PYTHONPATH=src python -c "import sys; from htr.cli import train_main; sys.argv=['htr-train']+sys.argv[1:]; train_main()" --config configs/baselines/4_01_line_crnn_ctc_digital_peter.yaml --epochs 60 --device cpu
